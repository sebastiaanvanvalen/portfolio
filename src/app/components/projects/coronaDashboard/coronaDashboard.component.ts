...
export class CoronaDashboardComponent implements OnInit {
    ...
    modalObject = {
        D3: {
            ...
            body: "The data in this model is transformed with the D3JS library. Pushing of sort types or selector buttons will rearange the bars, the scales and colors."
        },
        API: {
            title: "covid19api.com",
            body: "choosing a datasource for this project was not easy. It turned out many API's generate very large data packages with much data that I didn't need for this small project. This uses specific queries for very specific data.<br>This being such a small project I need to remind to check every now and then if the data is still displayed correctly and the API is still sending correct info. For instance one of the object key names was changed overnight.<br> The goal of this project was creating a nice model based on data available from a public API. Not creating a perfect situation dashboard concerning Covid. This means the situation will not be 100% complete. For instance, at the moment this is written, France has 0 recovered cases."
        },
    }
    ...
    constructor(private HttpService: HttpService, private TitleService: Title, private ModalService:ModalService) {
